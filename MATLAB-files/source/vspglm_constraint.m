function [ maxLogLike, param, converged] = vspglm_constraint(Y, X, links,Aeq, index,fixed)
...
%     end
    ...
    constraint = @(params) constraints2(params,X,y,dims,links, minMax, index, fixed);
    
    % Set options for FMINCON
    % 'interior-point'
    % 'active-set'
    % 'sqp[
%     options = optimset('MaxFunEvals',1e5, 'MaxIter', 1e4, 'TolFun', 1e-5,...
%              'TolCon', 1e-5, 'TolX', 1e-5,...
%              'Algorithm', 'interior-point',...
%              'Display', 'on', 'GradConstr', 'on', 'GradObj', 'on', ...
%              'SubproblemAlgorithm', 'cg', 'Hessian', 'fin-diff-grads') ;
    options = optimset('MaxFunEvals',1e5, 'MaxIter', 1e5, 'TolFun', 1e-6,...
