function [se, co, means, thetas, pexp, Yvar, samich_est, samich_se] = vcov(X, Y, param, links, tbl,formulas)
    ...
    %Try to see if we can get an interpolation of means
    % So for each observation, you want to compute the difference between Y
    % and it's mean.
    for i = 1:N
    ...
    for i = 1:N        
        ...
        meat = zeros(size(outerY{1}));
        for j = 1:N
            %t = t + pexp(i,j)*outerY{j};
            t = t + pexp(i,j)*(y(j,:).' - means(i, :).')*(y(j,:).' - means(i, :).').';
            meat = meat + (y(j,:).' - means(i, :).')*(y(j,:).' - means(i, :).').';
    ...
    for i = 1:N              
        Ycovs{i} = pinv(Yvar{i});
        %Ycovs{i} = inv(Yvar{i});
        %Ycovs{i} = pinv(outerY{i});
    end
   
   tbl = [tbl,  array2table(zeros(N, 1), 'VariableNames', {'test'})];
   ...
   samich = zeros(size((T{1})*(T{1}.')));
   for i = 1:N
       co = co + (T{i})*Ycovs{i}*(T{i}.');
       samich = samich + (T{i})*Ycovs{i}*(Ymeat{i})*Ycovs{i}*(T{i}.');
   end
   co = co;
   co = pinv(co);  
   samich_est = co * samich * co;
   samich_se = sqrt(abs(diag(samich_est)));
   %co = inv(co);
   se = sqrt(abs(diag(co)));
   %se = sqrt((diag(co)));
